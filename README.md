...
当前进度：**M2（二进制接入与时序处理）已完成**——在 M1（工具链约束、核心类型、配置、
日志、SQLite 迁移、文件发现/身份、CSV/JSONL 解析、基础校验、两阶段导入流水线、
归档/隔离、CLI）之上，新增 `libtlmcodec` C11 编解码库（帧/TLV/CRC32/zlib/损坏恢复）、
TLM 格式接入、校准、单位换算、去重（LRU+SQLite 权威）、水位线重排、缺失检测与插值、
固定窗口聚合（版本化 + 迟到修正）、滚动窗口统计，以及迁移 0002/0003。
M3（规则与 API）、M4（报表/性能）按里程碑推进。
